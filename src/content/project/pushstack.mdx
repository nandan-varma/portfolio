...
- **[@nandan-varma/git-fs-s3](https://www.npmjs.com/package/@nandan-varma/git-fs-s3)** — the S3/R2 filesystem adapter (with the caching and retry/circuit-breaker layers), the smart-HTTP protocol handler, and a higher-level ops layer (branches, commits, trees, history, diffs) built on top of it.
- **[@nandan-varma/git-edge](https://www.npmjs.com/package/@nandan-varma/git-edge)** — an object-level three-way merge that works without a worktree, plus a parsed-object LRU cache, for running isomorphic-git somewhere with no disk at all (Workers, edge functions).
...
- [@nandan-varma/git-fs-s3](https://www.npmjs.com/package/@nandan-varma/git-fs-s3) — R2 filesystem adapter, smart-HTTP protocol, ops layer
- [@nandan-varma/git-edge](https://www.npmjs.com/package/@nandan-varma/git-edge) — worktree-free merge, parsed-object cache
